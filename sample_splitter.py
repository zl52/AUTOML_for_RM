...
def sample_splitter(df, label=None, dt_col=None, val_size=0.2, test_size=0.2, split_by_ratio=True,
    ...
    """
    Split samples into two or three sets: train, test (validation sets).
    ...
    label: str
			Name of column indicating boolean label
    dt_col: str
			Name of column indicating datetime
    val_size: float
			Size of validation set (by ratio)
    test_size: float
			Size of oot(test) set
    split_by_ratio: boolean
			Split oot(test) set by defined ratio
    method: str
			If 'oot' is chosen, test set is derived based on datetime.
            If 'random' is chosen, test set is derived randomly.
    drop_dt_col: boolean
			Drop dt_col
    random_state: int
			Random seed
    ...
    """
    if not 0 <= val_size <= 1 or not 0 <= test_size <= 1 or not 0 <= val_size + test_size <= 1:
        raise ValueError('Invalid validation set size or test set size')
